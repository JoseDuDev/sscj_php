...
$select = mysql_query("SELECT * FROM online WHERE ip='".$ip."'");
if(isset($_SESSION['id_usuario'])){ $idu = $_SESSION['id_usuario']; } else { $idu = '0'; }
if(mysql_num_rows($select)>0) {
	mysql_query('UPDATE online SET time="'.time().'" WHERE ip="'.$ip.'"');
} else {
	mysql_query('INSERT INTO online (ip,time,id) VALUES ("'.$ip.'","'.time().'","'.$idu.'")');
}
mysql_query('DELETE FROM online WHERE time<'.(time()-(3*60)));
mysql_query('UPDATE usuarios SET time="" WHERE time<'.(time()-(3*60)));

if(isset($_SESSION['id_usuario'])){
	mysql_query('UPDATE usuarios SET time="'.time().'" WHERE id="'.$_SESSION['id_usuario'].'"');
}
...
//VARIÁVEIS PADRÃO
$vconfig = mysql_query("SELECT * FROM gerais WHERE id='1'");
while($row = mysql_fetch_object($vconfig)){
	$GLOBALS['config']["nome"]         = $row->nome;
	$GLOBALS['config']["url"]          = $row->url;
	$GLOBALS['config']["titulo"]       = $row->titulo;
	$GLOBALS['config']["des"]          = $row->des;
	$GLOBALS['config']["keyw"]         = $row->keyw;
	$GLOBALS['config']["email"]        = $row->email;
	$GLOBALS['config']["mascara"]      = $row->mascara;
	$GLOBALS['config']["telefone"]     = $row->telefone;
	$GLOBALS['config']["endereco"]     = $row->endereco;
	...
	$GLOBALS['config']["atende"]       = $row->atende;
    $GLOBALS['config']["msg1"]         = $row->msg;
    $GLOBALS['config']["msg2"]         = $row->msg1;
    $GLOBALS['config']["msg3"]         = $row->msg2;
    $GLOBALS['config']["msg4"]         = $row->msg3;
    $GLOBALS['config']["msg5"]         = $row->msg4;
}
...
if($inc=='noticias' && isset($_GET['url']) && !isset($_GET['local'])){
    ...
    $_SESSION['imagem_atual'] = 'img/logomarca-padrao-scj.jpg';
