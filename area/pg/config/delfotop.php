...
for($i=0; $i<count($dv); $i++){
	if(!empty($dv[$i])){
		$vr = mysql_fetch_object(mysql_query("SELECT foto FROM fotos_produtos WHERE id='".$dv[$i]."'"));
		unlink('../../'.$vr->foto);
		mysql_query("DELETE FROM fotos_produtos WHERE id='".$dv[$i]."'");
	}
}

$vf = mysql_query("SELECT * FROM fotos_produtos WHERE id_p='".$_POST['produto']."' ORDER BY id_o ASC");
