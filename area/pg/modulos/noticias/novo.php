...
if(isset($_GET['ativa'])) {
    ...
    header("Location: ?pg=modulos/noticias/novo#".$id);
    exit;
}       
if (isset($_GET['acao'])) {
    $acao = $_GET['acao'];    
    if ($acao == "add") {           
        ...
        if(isset($_FILES["arquivo"])){
            ...
            $insereCliente = mysql_query("INSERT INTO noticias(data_reg, imagem, id_usuario, fotografo, titulo, url_check, conteudo, fonte, status, introducao, categoria) 
                                                        VALUES('".$datar."', '$imagem', ".$_SESSION['id_usuario'].", '$fotografo', '$titulo', '".$dlxTools->remove_accents($titulo)."', '$conteudo', '$fonte', 'S', '".$introd."', '".$_POST['categoria']."')");
                                                        
            header("Location: ?pg=modulos/noticias/novo");
            exit;
        }
    }
    if($acao=="att"){
        $envio = '?pg=modulos/noticias/novo&acao=edit&id='.$_GET['id'];
        $bt  = 'Atualizar';
    ...
    $envio = '?pg=modulos/noticias/novo&acao=add';
    $bt = 'Cadastrar';
    $dpg = 'Gerenciamento de Noticias';
    $img       = '';
    $fotografo = '';
...
"bold,italic,underline,strikethrough,justifyleft,justifycenter,justifyright,justifyfull,link,unlink,image,media,table,formatselect,fontselect,fontsizeselect,forecolor,backcolor",
...
<table width="95%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <th scope="col" width="10%" align="right"><a href='?pg=modulos/noticias/direciona'><img src='img/icones/voltar.png' width='32' height='32' alt='Painel de Controle' /></a></th>
  </tr>
</table>
    ...
    <tr>
      <td height="36" align="right" valign="middle"> Categoria&nbsp;&nbsp;</td>
      <td height="36" valign="top" align="left">
          <select name="categoria" style="height: 30px; width: 460px;">
            <?php
            if($categoria=='' || $categoria=='0'){
                $q_select = mysql_query("SELECT id, nome FROM categoria WHERE status = 'S' ORDER BY id_o ASC");
            } else {
                $q_select = mysql_query("SELECT id, nome FROM categoria WHERE id!='".$categoria."' AND status = 'S' ORDER BY id_o ASC");
                echo '<option value="'.$categoria.'">Atual -- '.mysql_result(mysql_query("SELECT nome FROM categoria WHERE id='".$categoria."'"), 0, "nome").'</option>';
            }
            while( $sel_est = mysql_fetch_array($q_select) ) {
                echo '<option value="'.$sel_est['id'].'">'.$sel_est['nome'].'</option>';
            }            
            ?>
          </select>
      </td>
...
<?php
if(!isset($_GET['acao'])){
    $sql = mysql_query("SELECT * FROM noticias ORDER BY data_reg DESC");
    if(mysql_num_rows($sql)==0) {
        ...
        while($linha = mysql_fetch_array($sql)) {
            ...
            echo "<td>";
            echo "
            <a href='?pg=modulos/noticias/novo&ativa=$linha[0]' name='".$linha[0]."'><img src='img/icones/status.png' width='24' heigth='24' alt='Ativar/Desativar' title='Ativar/Desativar' /></a>
            <a onclick=\"javascript:confirmaDel('?pg=modulos/noticias/novo&del=$linha[0]')\" style='cursor:pointer;'><img src='img/icones/deletar.png' width='24' heigth='24' alt='Remover' title='Remover' /></a>
            <a href='?pg=modulos/noticias/novo&acao=att&id=$linha[0]'><img src='img/icones/editar.png' width='24' heigth='24' alt='Alterar' title='Alterar' /></a>";
            echo "</td>";
            echo "</tr>";
        }
        echo '</tbody>';
