			...
			<div class="slider">
				<div class="sliderContent">';
					while($dados = mysql_fetch_object($selb)){
					    ...
					    if($dados->site != "") {
						   echo "<a href='http://".$dados->site."' target='_blank'>";
					    }
		...
		<ul>
			...
			for( $i=0; $i<count($nots); $i++ ){
				...
				echo '<li>';
					...
					echo '</div>';
					echo '<div class="image"><img src="http://'.$nots[$i]['local'].'area/'.$nots[$i]['imagem'].'" width="180" height="120" alt="'.$nots[$i]['titulo'].'" title="'.$nots[$i]['titulo'].'" /></div>
					<div class="contexto">';
