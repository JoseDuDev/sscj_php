...
if($valida == "enviar"){	
	...
	$ass = $anome.", ".$nome." te indicou o santo do dia.";
	...
	$msg .= "<strong>".$nome."</strong> está indicando o santo do dia ".$_POST["tnot"]."<br /><br />";
...
<?php
$dv = explode('-',$_GET['data']);
$data = $dv[2].'/'.$dv[1];
$vdc = mysql_query("SELECT * FROM santododia WHERE data_reg='".$data."'");
...
<div class="paginas">
	<div class="util">
		<div class="area_noticias">
			<div class="intro3">SANTO DO DIA - <?php echo $data; ?></div>
		</div>
	</div>
	<div class="relaciona">
		...
		<div id="calendario">
			<div class="topo">
				<a href="./interatividade/santo-do-dia/<?php echo $anoa.'-'.$mesa.'-'.$dia; ?>"><div class="mesanterior"><?php echo substr(escreveMes($mesa),0,3); ?></div></a>
				<div class="mesatual"><?php echo escreveMes($mes).' '.$ano; ?></div>
				<a href="./interatividade/santo-do-dia/<?php echo $anop.'-'.$mesp.'-'.$dia; ?>"><div class="mesposterior"><?php echo substr(escreveMes($mesp),0,3); ?></div></a>
			</div>
			<div class="dias">
				<li>DOM</li>
				<li>SEG</li>
			...
			<div class="uteis">
				...
				for($i=1; $i<=($limite + ($comeca - 1)); $i++){
					if($i>=$comeca){
						$c++;
						if(strlen($c)==1){ $c = '0'.$c; }
						echo '<a href="./interatividade/santo-do-dia/'.$ano.'-'.$mes.'-'.$c.'" class="escrito"><li '; if($dia==$c){ echo ' class="ativo" '; } echo '>'.$c.'</li>';
					} else {
			...
			<a href="./interatividade/santo-do-dia/<?php echo date("Y-m-d"); ?>"><div class="hoje">HOJE, <?php echo date("d/m/Y"); ?></div></a>
		</div>
	...
	<div class="util">
		<div class="utiln">
			...
			<div class="noticiaAtivo">
				<div class="propNoti">
					<?php echo '<img src="https://www.agenciaarcanjo.com.br/area/'.$row->imagem.'" width="300" alt="'.$row->nome.'" style="float: left; padding: 0px 20px 20px 0px;" title="'.$row->nome.'" />'.nl2br($row->descricao).'</a>'; ?>
					<div class="clear"></div>
				</div>
				<div class="clear"></div>
			</div>
			...
			<div class="divulgacao">
				<a href="https://www.facebook.com/share.php?u=https://<?php echo $_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI']; ?>&t=<?php echo urlencode($row->nome); ?>" target="_blank"><div class="compartilhe"><img src="img/bt-compartilhe.png" width="151" height="38" alt="Compartilhe está Notícia" title="Compartilhe está Notícia" /></div></a>
				<div onclick="window.print();" class="imprimir"></div>
				<div onclick="indica();" class="indicar"></div>
				<a href="https://www.facebook.com/share.php?u=https://<?php echo $_SERVER['SERVER_NAME'].$_SERVER ['REQUEST_URI']; ?>&t=<?php echo urlencode($row->nome); ?>" target="_blank"><div class="face"></div></a>
			</div>

			<div id="dados_indicacao">
				<div class="intro6">Indique a um amigo</div>
				<div class="clear"></div>
				<form name="indicao" id="indicacao" action="./?pg=santos&indica" method="post">
					...
					<input type="hidden" name="endereco" id="endereco" value="https://<?php echo $GLOBALS['config']['url'].'/'.$_GET['url'].'/'.$_GET['noticia']; ?>" />
